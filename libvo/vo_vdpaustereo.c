...
static vo_info_t info = {
    ...
    "vdpaustereo",
    "Cristian Rocha <cristian.rocha@moldeointeractive.com.ar> and others",
    ""
};
...
/* Declaration for all variables of win_x11s_init_vdpau_procs() and
 * win_x11s_init_vdpau_flip_queue() functions
 */
...
#define MOLDEO_SIDE ((vo_frame & 0x1)? LEFT: RIGHT)
...
/* Use 3 references for stereoscopic */
#define MAX_REFS	3
...
{
	video_to_output_surface_S(&VdpStream[MOLDEO_SIDE]);
}
...
static void resize(void)
{
    ...
    mp_msg(MSGT_VO, MSGL_ERR, "[vdpaustereo] resize (%i, %i) {\n", vid_width, vid_height);
    ...
    mp_msg(MSGT_VO, MSGL_ERR, "[vdpaustereo] out_rect_vid (%i, %i, %i, %i) {\n", out_rect_vid.x0, out_rect_vid.x1, out_rect_vid.y0, out_rect_vid.y1 );
    mp_msg(MSGT_VO, MSGL_ERR, "[vdpaustereo] src_rect_vid (%i, %i, %i, %i) {\n", src_rect_vid.x0, src_rect_vid.x1, src_rect_vid.y0, src_rect_vid.y1 );
...
static int win_x11s_init_vdpau_procs(void)
{
    VdpStatus vdp_st;
    ...
    mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: win_x11s_init_vdpau_procs {\n");
    ...
    /* Bloquea el proceso si el sistema de ventanas lo exige. 
     * Permite reiniciar la configuración del vdpau una vez que vuelve */
...
static int win_x11s_init_vdpau_flip_queue_S(tScreenOutput *Screen, tVdpStream *VdpStream);

static int win_x11s_init_vdpau_flip_queue(void)
{
	int r;
	mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: win_x11s_init_vdpau_flip_queue {\n");
	r = win_x11s_init_vdpau_flip_queue_S(&ScreenLeft, &VdpStream[LEFT]);
	r = win_x11s_init_vdpau_flip_queue_S(&ScreenRight, &VdpStream[RIGHT]);
	mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: win_x11s_init_vdpau_flip_queue }\n");
	return r;
}

static int win_x11s_init_vdpau_flip_queue_S(tScreenOutput *Screen, tVdpStream *VS)
{
    VdpStatus vdp_st;
...
static int handle_preemption(void)
{
    ...
    if (win_x11s_init_vdpau_procs() < 0 ||
        win_x11s_init_vdpau_flip_queue() < 0 ||
                  ...
                  uint32_t format)
{
...
#endif
    {
...
#endif
        XGetWindowAttributes(ScreenLeft.display, DefaultRootWindow(ScreenLeft.display), &attribs);
        depth = attribs.depth;
        ...
        vo_x11s_create_vo_window_S(&ScreenLeft, vo_dx, vo_dy, d_width, d_height,
                                flags, CopyFromParent, "vdpaustereo", title);
        XChangeWindowAttributes(ScreenLeft.display, ScreenLeft.window, xswamask, &xswa);
        vo_x11s_create_vo_window_S(&ScreenRight, vo_dx, vo_dy, d_width, d_height,
                                flags, CopyFromParent, "vdpaustereo", title);
    ...
    if (VdpStream[LEFT].vdp_flip_queue == VDP_INVALID_HANDLE &&
		    win_x11s_init_vdpau_flip_queue_S(&ScreenLeft, &VdpStream[LEFT]))
        return -1;
    mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] RIGHT\n");
    if (VdpStream[RIGHT].vdp_flip_queue == VDP_INVALID_HANDLE &&
		    win_x11s_init_vdpau_flip_queue_S(&ScreenRight, &VdpStream[RIGHT]))
        return -1;
    mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: Setup VdpStream }\n");
    ...
    VdpStream[0].surface_num = -1;
    VdpStream[1].surface_num = -1;

    surface_num     =  0;
    vid_surface_num = -1;
    resize();

    mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: config }\n");
    return 0;
}

/* Chequea eventos en la ventana */
...
{
    VdpStatus vdp_st;
    int e = vo_x11s_check_events_S(Screen);
    ...
    if ((e & VO_EVENT_EXPOSE || e & VO_EVENT_RESIZE) && int_pause) {
        ...
        if (visible_buf) {
            mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] EXPOSE %i %i \n", vo_frame, surface_num);
...
{
    mp_msg(MSGT_VO, MSGL_DBG2, "draw_osd [f:%i]\n", vo_frame);
...
/* Intercambia buffer por pantalla */
static void flip_page(void)
{
    VdpStatus vdp_st;

    mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: flip_page [f:%i] {\n", vo_frame);

    mp_msg(MSGT_VO, MSGL_DBG2, "\nFLIP_PAGE VID:%u -> OUT:%u (%i)\n",
           surface_render[vid_surface_num].surface, output_surfaces[surface_num], MOLDEO_SIDE);

    if (handle_preemption() < 0)
        return;

    //if (VdpStream[0].surface_num > 0 && VdpStream[1].surface_num > 0 && MOLDEO_SIDE) {
    if (MOLDEO_SIDE) {
        mp_msg(MSGT_VO, MSGL_DBG2, "FLIP PAGES\n");
    	vdp_st = vdp_presentation_queue_display(VdpStream[0].vdp_flip_queue,
			output_surfaces[VdpStream[0].surface_num],
			vo_dwidth, vo_dheight, 0);
    	vdp_st = vdp_presentation_queue_display(VdpStream[1].vdp_flip_queue,
			output_surfaces[VdpStream[1].surface_num],
			vo_dwidth, vo_dheight, 0);
        visible_buf = 1;
    }
    surface_num = (surface_num + 1) % NUM_OUTPUT_SURFACES;
    VdpStream[MOLDEO_SIDE].surface_num = surface_num;
                      ...
                      int x, int y)
{
    ...
    int max_refs = image_format == IMGFMT_VDPAU_H264 ? rndr->info.h264.num_ref_frames : MAX_REFS;
...
{
    mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: draw_image [f:%i] {\n" , vo_frame);
...
static uint32_t get_image(mp_image_t *mpi)
{
    ...
    mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: get_image [f:%i] {\n", vo_frame);
...
static int preinit(const char *arg)
{
  ...
  mp_msg(MSGT_VO, MSGL_DBG2, "[vdpaustereo] MOLDEO: preinit {\n");
    ...
    if (!vo_s_init() || win_x11s_init_vdpau_procs())
        return -1;
...
{
    mp_msg(MSGT_VO, MSGL_DBG2, "control [f:%i]\n", vo_frame);
